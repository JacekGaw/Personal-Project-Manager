...
const AddNoteForm = ({ onAddProject, projectID, noteEdit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if(projectID){
      assignID = projectID;
      assignTitle = projects.filter(project => project.id === projectID)[0].Title;
    }
    else if(assignRef.current.value !== "none"){
      assignID = assignRef.current.value;
      assignTitle = projects.filter(project => project.id === assignID)[0].Title;
    }
    ...
    try {
        setDisabled(true);
        if(noteEdit){
          await editNote(noteEdit.id, titleRef.current.value, noteRef.current.value, assignID, assignTitle);
        }
        else if(!noteEdit){
          await addNote(titleRef.current.value, noteRef.current.value, assignID, assignTitle);
        }
        setDisabled(false);
      } catch (err) { console.log(err); }
    titleRef.current.value = "";
    noteRef.current.value = "";
    if(!projectID) {assignRef.current.value = "none"};
    onAddProject();
  };
    ...
    <header className="p-2 mb-2">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 mb-4">
        ...
        <label
          ...
          <select id="note-assign" className="text-sm text-slate-700 font-[400]" ref={assignRef} defaultValue={noteEdit && noteEdit.assign}>
        ...
        <Button type="submit" disabled={disabled}>{noteEdit ? "Update Note" : "Add Note"}</Button>
      </form>
    </>
  );
};
